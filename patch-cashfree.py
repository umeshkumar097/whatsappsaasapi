...
# Remove the 'import Cashfree from "cashfree";' line
content = content.replace('import Cashfree from "cashfree";\n', '')
content = content.replace('let cashfreeInstance: any | null = null;\n', '')
content = content.replace('cashfreeInstance = null;\n', '')
# Import axios if not present
...
start_match = re.search(r'export async function getCashfree\(\).*?\{', content, re.DOTALL)
end_match = re.search(r'export async function getCashfreeKeyId\(\): Promise<string \| null> \{.*?\n\}\n', content, re.DOTALL)
