...
async function main() {
  ...
  try {
    ...
    console.log("No Auth URL:", res.data.subscription_session_id);
  } catch(e: any) { 
    console.log("Error1:", e.response?.data || e.message);
  }
  ...
  try {
    const res = await axios({
      ...
      data: {
        subscription_id: "test_auth_" + Date.now(),
        plan_details: { 
          plan_name: "Test Auth",
          plan_type: "PERIODIC",
          plan_currency: "INR",
    ...
    console.log("Error2:", e.response?.data || e.message);
  }
